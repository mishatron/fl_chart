...
abstract class AxisChartPainter<D extends AxisChartData>
  ...
  void _drawGrid(Canvas canvas, Size viewSize) {
    ...
    if (data.gridData.drawVerticalLine) {
      ...
      final end = lastPositionOverlapsWithBorder
          ? data.maxX - data.gridData.verticalInterval
          : data.maxX;
      ...
      while (verticalSeek <= end) {
        if (data.gridData.checkToShowVerticalLine(verticalSeek)) {
          ...
          canvas.drawDashedLine(Offset(x1, y1), Offset(x2, y2), _gridPaint,
    ...
    if (data.gridData.drawHorizontalLine) {
      ...
      final end = lastPositionOverlapsWithBorder
          ? data.maxY - data.gridData.horizontalInterval
          : data.maxY;
  ...
  void _drawBackground(Canvas canvas, Size viewSize) {
    if (data.backgroundColors == null || data.backgroundColors.isEmpty) {
      return;
    }

    final Size usableViewSize = getChartUsableDrawSize(viewSize);
    final double backPartHeight =
        usableViewSize.height / data.backgroundColors.length;

    for (int i = 0; i < data.backgroundColors.length; ++i) {
      _backgroundPaint.color = data.backgroundColors[i];
      canvas.drawRect(
        Rect.fromLTWH(
          getLeftOffsetDrawSize(),
          getTopOffsetDrawSize() + i * backPartHeight,
